...
export async function finalizeSale(input: z.infer<typeof saleSchema>, employeeId: string) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const customerQuery = input.customer
      ? db.collection("customers").where("phone", "==", input.customer.phone).limit(1)
      : null;

    // Firestore requires every transaction read to happen before the first write.
    // Read the products, receipt counter, and optional customer together so a sale
    // with customer details follows the same valid transaction path as a walk-in.
    const [docs, counter, matchingCustomer, businessSettings] = await Promise.all([
      Promise.all(refs.map((ref) => transaction.get(ref))),
      transaction.get(counterRef),
      customerQuery ? transaction.get(customerQuery) : Promise.resolve(null),
      transaction.get(businessSettingsRef),
    ]);
    const maxDiscountPercent = businessSettings.data()?.maxDiscountPercent ?? 100;
    const lines = input.lines.map((line, index) => {
      ...
      const minimumPrice = standardPriceAtSale * (1 - maxDiscountPercent / 100);
      if (line.actualSellingPrice < minimumPrice) throw new Error(`The price for ${product.name} exceeds the maximum allowed discount of ${maxDiscountPercent}%.`);
    ...
    const receiptSequence = (counter.data()?.receiptSequence ?? 0) + 1;
    const receiptPrefix = businessSettings.data()?.receiptPrefix ?? "VX";
    const receiptNumber = `${receiptPrefix}-${new Date().toISOString().slice(0, 10).replaceAll("-", "")}-${String(receiptSequence).padStart(4, "0")}`;
    let customerRef: FirebaseFirestore.DocumentReference | null = null;
    if (input.customer && matchingCustomer) { customerRef = matchingCustomer.empty ? db.collection("customers").doc() : matchingCustomer.docs[0].ref; transaction.set(customerRef, { ...input.customer, updatedAt: FieldValue.serverTimestamp(), ...(matchingCustomer.empty ? { createdAt: FieldValue.serverTimestamp(), totalPurchases: 0, outstandingBalance: 0 } : {}) }, { merge: true }); }
    const saleRef = db.collection("sales").doc();
    transaction.set(counterRef, { receiptSequence, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
    transaction.set(saleRef, { receiptNumber, employeeId, customerId: customerRef?.id ?? null, customerName: input.customer?.name ?? null, customerPhone: input.customer?.phone ?? null, source: "POS", status: "COMPLETED", paymentStatus: balance === 0 ? "PAID" : paid > 0 ? "PARTIALLY_PAID" : "UNPAID", lines, total, cogs: lines.reduce((sum, line) => sum + line.unitCostAtSale * line.quantity, 0), grossProfit: lines.reduce((sum, line) => sum + line.profitAtSale, 0), paidAmount: paid, outstandingBalance: balance, notes: input.notes ?? null, createdAt: FieldValue.serverTimestamp() });
