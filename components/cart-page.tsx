...
export function CartPage() {
  ...
  const changeQuantity = (productId: string, quantity: number) => { const next = quantity < 1 ? lines.filter((line) => line.productId !== productId) : lines.map((line) => line.productId === productId ? { ...line, quantity } : line); setLines(next); writeCart(next); };
